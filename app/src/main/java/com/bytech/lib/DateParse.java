...
public class DateParse {
    ...
    public String parseString() {
        ...
        return date[2] + " " + month[Integer.parseInt(date[1].trim())] + " " +
                date[0] + " jam " + time[0] + ":" + time[1];

    }

    public String parseNoTime() {

        String[] date = strDate.split("[-]");

        String[] month = {"", "Jan", "Feb", "Mar", "Apr", "May", "Jun", "Jul", "Aug", "Sep", "Oct", "Nov", "Dec"};

        return date[2] + " " + month[Integer.parseInt(date[1].trim())] + " " + date[0];
    }
}
